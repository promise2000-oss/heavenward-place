...
export default function PropertyCard({ property, className, variant = "default" }: PropertyCardProps) {
    ...
    return (
      <Link href={`/property/${property.slug}`} className={cn("group flex flex-col sm:flex-row bg-white rounded-2xl overflow-hidden premium-card shadow-sm", className)}>
        <div className="relative w-full sm:w-72 h-52 sm:h-auto overflow-hidden">
          <Image src={property.images[0]?.url || "/images/properties/1/cover.jpg"} alt={property.images[0]?.alt || property.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700 ease-out-expo" sizes="(max-width: 640px) 100vw, 288px" />
        ...
        <div className="flex-1 p-6">
          ...
          <div className="flex items-center gap-1.5 mt-2 text-sm text-charcoal">
          ...
          <p className="mt-3 text-sm text-charcoal line-clamp-2 leading-relaxed">{property.shortDescription}</p>
  ...
  if (variant === "compact") {
    return (
      <Link href={`/property/${property.slug}`} className={cn("group flex items-center gap-4 p-4 rounded-xl hover:bg-primary-lighter/30 transition-all duration-300", className)}>
  ...
  return (
    <Link href={`/property/${property.slug}`} className={cn("group block bg-white rounded-2xl overflow-hidden premium-card shadow-sm", className)}>
      <div className="relative aspect-[4/3] overflow-hidden">
        <Image src={property.images[0]?.url || "/images/properties/1/cover.jpg"} alt={property.images[0]?.alt || property.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700 ease-out-expo" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        
        <button onClick={(e) => { e.preventDefault(); setIsSaved(!isSaved); }} className="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white transition-all duration-300 cursor-pointer z-10 shadow-sm" aria-label={isSaved ? "Remove from wishlist" : "Add to wishlist"}>
          <Heart size={18} className={cn("transition-all duration-300", isSaved ? "fill-error text-error scale-110" : "text-ink/60 group-hover:text-error")} />
        </button>
        
        {property.isNew && <span className="absolute top-4 left-4 px-4 py-1.5 text-xs font-semibold gradient-animated text-white rounded-full shadow-lg z-10">New</span>}
        {property.isFeatured && !property.isNew && <span className="absolute top-4 left-4 px-4 py-1.5 text-xs font-semibold bg-secondary/90 text-white rounded-full shadow-lg z-10">Featured</span>}
      </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-3 mt-4 text-sm text-charcoal">
        ...
        <div className="mt-5 pt-4 border-t border-border/60 flex items-baseline gap-1">
          <span className="text-xl font-bold text-primary">{formatPrice(property.pricing.nightlyRate, property.pricing.currency)}</span>
