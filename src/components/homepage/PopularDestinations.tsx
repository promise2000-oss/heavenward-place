      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
          <motion.div variants={fadeInUp} className="text-center mb-14">
            <span className="inline-flex items-center gap-2 px-4 py-2 glass-card rounded-full text-sm font-medium text-primary mb-6">
              <span className="w-1.5 h-1.5 bg-neon-teal rounded-full animate-pulse-glow" />
              Destinations
            </span>
            ...
            {cities.map((city) => (
              <motion.div key={city.id} variants={fadeInUp}>
                <Link href={`/search?city=${city.slug}`} className="group relative block h-80 rounded-2xl overflow-hidden premium-card">
                  <Image src={city.image} alt={city.name} fill className="object-cover group-hover:scale-110 transition-transform duration-700 ease-out-expo" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-7">
                    ...
                    <p className="mt-1 text-sm text-white/80">{city.propertyCount} premium properties</p>
                    <div className="mt-4 w-0 group-hover:w-12 h-0.5 bg-gradient-to-r from-neon-teal to-transparent transition-all duration-500" />
                  </div>
                </Link>
              </motion.div>
            ))}
